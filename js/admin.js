// ===== Admin Panel Application - Complete with Bulk Fix + No Polling =====
...
const AdminApp = {
        ...
        container.innerHTML = filtered.map(topup => {
            ...
            return `<div class="topup-card">
                <div class="topup-header">
                    <div class="topup-user"><img src="${user?.photoUrl || this.getAvatar(topup.telegramId)}" alt="User"><div><h4>${user?.firstName || 'User'} ${user?.lastName || ''}</h4><p>@${user?.username || 'N/A'}</p></div></div>
                    <span class="status-badge ${topup.status}">${topup.status}</span>
                </div>
                <div class="topup-body">
                    <div class="topup-amount"><span>Amount</span><strong>${Utils.formatCurrency(topup.amount, 'MMK')}</strong></div>
                    <div class="topup-method"><span>Payment Method</span><strong>${topup.paymentMethod}</strong></div>
                    <div class="topup-date"><i class="fas fa-clock"></i> ${Utils.formatDate(topup.createdAt, 'long')}</div>
                    ${topup.proofImage ? `<div class="topup-proof"><img src="${topup.proofImage}" alt="Payment Proof" onclick="window.open('${topup.proofImage}', '_blank')"></div>` : ''}
                </div>
                ${topup.status === 'pending' ? `<div class="topup-actions">
                    <button class="btn btn-success" onclick="AdminApp.approveTopup('${topup.id}')"><i class="fas fa-check"></i> Approve</button>
                    <button class="btn btn-danger" onclick="AdminApp.rejectTopup('${topup.id}')"><i class="fas fa-times"></i> Reject</button>
                </div>` : ''}
            </div>`;
        }).join('');
    },
    ...
    async bulkDeleteProducts() {
        ...
        try {
            ...
            // Read once, filter, write once
            const data = await Database.readFresh(Database.bins.PRODUCTS);
            const products = data?.products || [];
            const filtered = products.filter(p => !idsToDelete.includes(p.id));
            await Database.update(Database.bins.PRODUCTS, { products: filtered });
            // Update local state
    ...
    async openG2BulkServiceBrowser() {
        ...
        if (this.state.g2bulkServicesRaw.length === 0) {
            ...
            catch (e) { document.getElementById('g2bulk-browser-list').innerHTML = '<p>Failed to load</p>'; return; }
        }
    ...
    renderBrowserServices(services) {
        ...
        container.innerHTML = services.slice(0, 100).map(s => `<div class="g2bulk-browser-item" onclick="AdminApp.selectBrowserService(${s.service})"><div class="service-id">#${s.service}</div><div class="service-info"><strong>${s.name}</strong><br><small>$${s.rate} | Min: ${s.min} | Max: ${s.max}</small></div><button class="btn btn-sm btn-primary"><i class="fas fa-check"></i></button></div>`).join('');
    },
    ...
    // ===== ★★★ BULK IMPORT FROM G2BULK (FIXED - Single DB Write) ★★★ =====
    ...
    async loadBulkImportServices() {
        if (this.state.g2bulkServicesRaw.length === 0) {
            ...
            } catch (e) { Utils.showToast('Failed to load services', 'error'); Utils.hideLoading(); return; }
            Utils.hideLoading();
        }
    ...
    // ★★★ FIXED: Read DB once, build all products, write once ★★★
    async saveBulkImport() {
        ...
        try {
            ...
            // 2. Read products DB once
            let dbData;
            try { dbData = await Database.readFresh(Database.bins.PRODUCTS); }
            catch (e) { dbData = { products: [] }; }
            const existingProducts = dbData?.products || [];

            // 3. Build all new products in memory
            const selectedServices = selectedIds.map(id => this.state.g2bulkServicesRaw.find(s => s.service === id)).filter(Boolean);
            const newProducts = [];
            ...
            for (const service of selectedServices) {
                ...
                const newProduct = {
                    ...
                    exchangeRate: rate,
                    isActive: true,
                    order: existingProducts.length + newProducts.length,
                    sold: 0,
            ...
            // 4. Write all at once (single DB call)
            const allProducts = [...existingProducts, ...newProducts];
            await Database.update(Database.bins.PRODUCTS, { products: allProducts });
            ...
            Utils.showToast(`Successfully imported ${newProducts.length} products!`, 'success');
        } catch (error) {
    ...
    // ===== ★★★ BULK PRICE UPDATE (FIXED - Single DB Write) ★★★ =====
    ...
    updateBulkPriceUI() {
        ...
        const categoryId = document.getElementById('bulk-price-category').value;
        if (categoryId) this.renderBulkPriceProductsList(this.state.products.filter(p => p.categoryId === categoryId));
    ...
    // ★★★ FIXED: Read DB once, update all in memory, write once ★★★
    async saveBulkPriceUpdate() {
        ...
        try {
            // 1. Read products DB once
            const dbData = await Database.readFresh(Database.bins.PRODUCTS);
            const allProducts = dbData?.products || [];

            let successCount = 0;
            let skippedCount = 0;

            // 2. Update prices in memory
            for (const productId of selectedIds) {
                const dbIdx = allProducts.findIndex(p => p.id === productId);
                if (dbIdx === -1) continue;

                const usdRate = parseFloat(allProducts[dbIdx].g2bulkRate);
                if (!usdRate || usdRate <= 0) { skippedCount++; continue; }

                const newPrice = Math.ceil(usdRate * rate);
                const discount = allProducts[dbIdx].discount || 0;
                allProducts[dbIdx].price = newPrice;
                allProducts[dbIdx].discountedPrice = discount > 0 ? Math.round(newPrice - newPrice * discount / 100) : newPrice;
                allProducts[dbIdx].currency = 'MMK';
                allProducts[dbIdx].exchangeRate = rate;
                allProducts[dbIdx].updatedAt = new Date().toISOString();
                successCount++;
            }
    ...
    renderG2BulkServicesList(services) {
        ...
        if (services.length === 0) { container.innerHTML = '<div class="empty-state"><i class="fas fa-search"></i><p>No services found</p></div>'; return; }
        const display = services.slice(0, 200);
        container.innerHTML = display.map(service => `<div class="g2bulk-service-card"><div class="service-header"><span class="service-id-badge">#${service.service}</span><span class="service-category-badge">${service.category || 'N/A'}</span></div><div class="service-name">${service.name}</div><div class="service-details"><div class="service-detail"><span>Rate:</span><strong>$${service.rate}</strong></div><div class="service-detail"><span>Min:</span><strong>${service.min}</strong></div><div class="service-detail"><span>Max:</span><strong>${service.max}</strong></div></div><button class="btn btn-primary btn-sm btn-full" onclick="AdminApp.quickAddProduct(${service.service})"><i class="fas fa-plus"></i> Add as Product</button></div>`).join('');
    ...
    async deleteBanner(bannerId, type) {
        ...
        try {
            ...
            this.renderBanners(); Utils.showToast('Banner deleted!', 'success');
        } catch (error) { Utils.showToast('Failed to delete', 'error'); }
        finally { Utils.hideLoading(); }
    },
    ...
    loadSampleConfig(type) {
        ...
        if (config) { document.getElementById('checker-json-config').value = config; this.previewCheckerConfig(); Utils.showToast('Sample config loaded! Remember to replace YOUR_RAPIDAPI_KEY_HERE', 'info'); }
    },
    ...
    async deleteInputTable(tableId) {
        ...
        Utils.showLoading('Deleting...');
        try { await Database.deleteInputTable(tableId); this.state.inputTables = this.state.inputTables.filter(t => t.id !== tableId); this.renderInputTables(); Utils.showToast('Deleted!', 'success'); }
    ...
    async deletePayment(paymentId) {
        ...
        Utils.showLoading('Deleting...');
        try { await Database.deletePaymentMethod(paymentId); this.state.payments = this.state.payments.filter(p => p.id !== paymentId); this.renderPayments(); Utils.showToast('Deleted!', 'success'); }
        catch (error) { Utils.showToast('Failed', 'error'); }
        finally { Utils.hideLoading(); }
    },
    ...
    async unbanUser(telegramId) {
        ...
        try { await Database.unbanUser(telegramId); await TelegramBot.notifyUnban(telegramId); this.state.bannedUsers = this.state.bannedUsers.filter(u => u.telegramId !== telegramId); this.renderBannedUsers(); Utils.showToast('User unbanned!', 'success'); }
        catch (error) { Utils.showToast('Failed', 'error'); }
        finally { Utils.hideLoading(); }
    },
    ...
    async saveSettings() {
        ...
        try {
            ...
            if (logoInput.files[0]) updates.websiteLogo = await this.uploadToImgbb(logoInput.files[0]);
